...
class EEGActivationGenerator(ActivationGeneratorBase):
    def __init__(self, model, source_dir, acts_dir, max_examples=10):
        self.source_dir = source_dir #where EEG data is stored examples (cnocepts, class, random)
    ...
    def get_examples_for_concept(self, concept):
        ...
        eeg_exp = self.load_eegs_from_files(eeg_paths = eeg_paths_list, max_examples = self.max_examples) #what is eeg shape?

        return eeg_exp

    ##þarf ekki eins og er, nema við viljum gera flóknari aðferð seinna
    def load_eeg_from_file(self, filename): #eeg shape?   
        ...
        #try:
        # 
        # load in from given filename time window raw edf files USING MNE?
        # 
        # return the eeg           
    ...
    def load_eegs_from_files(self, eeg_paths, max_examples):
        ...
        for eeg in eeg_paths : 
            ...
            #sumir eru 1535
            if(eeg_tensor_new.size(2) != 1536) : 
                 eeg_tensor_new = Ftorch.pad(input=eeg_tensor_new, pad=(0, 1, 0, 0), mode='constant', value=0)
            eeg_tensor = torch.cat((eeg_tensor, eeg_tensor_new), 0)
        print("tensor loaded for path:")
        print(eeg_paths)
        print(eeg_tensor.size())

        return eeg_tensor.to(dtype=torch.float32)

        # for each file name
        #    append to array.... RETURN what BENDR needs as input
