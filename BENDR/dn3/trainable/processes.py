    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None,
        ...
        def print_training_metrics(epoch, iteration=None):
            if iteration is not None:
                ...
                for m in metrics:
                    whatMetric = "train_" + m 
                    wandb.log({whatMetric: metrics[m]}, step = config_myglobals.WhatFold*config_myglobals.TotalNrOfEpochs + epoch)
        ...
        def _validation(epoch, iteration=None):
            ...
            if iteration is not None:
                ...
                for m in _metrics:
                    whatMetric = "val_" + m 
                    wandb.log({whatMetric: _metrics[m]}, step = config_myglobals.WhatFold*config_myglobals.TotalNrOfEpochs + epoch)
        ...
        for epoch in epoch_bar:
            self.epoch = epoch
            config_myglobals.WhatEpochIsIt = epoch
            ...
            config_myglobals.WhatEpochWasIt = epoch
...
class StandardClassification(BaseProcess):
    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None, epoch_callback=None,
        ...
        print("INN Í STANDARD CLASSIFICATION - FIT(TRAIN,VAL)")
        print("Training targets:")
        print(training_dataset.get_targets())
        print(len(training_dataset.get_targets()))
        config_myglobals.TargetOutputsTrain = training_dataset.get_targets()[0:int(np.floor(len(training_dataset.get_targets())/batch_size)*batch_size)]
        print("after floor:")
        print(len(config_myglobals.TargetOutputsTrain))
        print("INN Í STANDARD CLASSIFICATION - FIT(TRAIN,VAL)")
        print("Val targets:")
        print(validation_dataset.get_targets())
        print(len(validation_dataset.get_targets()))
        config_myglobals.TargetOutputsVal = validation_dataset.get_targets()
    ...
    def _make_dataloader(self, dataset, training=False, **loader_kwargs):
        ...
        print("INNI Í _make_dataloader - process.py")
        print(dataset.get_targets())
        print(len(dataset.get_targets()))
