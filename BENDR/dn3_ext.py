...
class LinearHeadBENDR(Classifier):
    ...
    def features_forward(self, x):
        #encode-ar
        print("INPUT SIZE INN Í FORWARD:")
        print(type(x))
        print(x.size())
        x = self.encoder(x)

        # testerUpdate2 = torch.cat((testerUpdate, tester2), 0)

        #maskar bendrs
        x = self.enc_augment(x)
        # tekur inn alla bendera og skilar 4
        x = self.summarizer(x)
        #print(x.Flatten())
        #setur inn í classifierinn
        TransformerOutput_new  = self.extended_classifier(x)


        if(config_myglobals.WhatEpochIsIt != config_myglobals.WhatEpochWasIt ) :

            if(not(config_myglobals.WhatEpochIsIt == 1 and config_myglobals.WhatFold == 0)) : 

                print("Updating plots for Epoch {}".format(config_myglobals.WhatEpochWasIt))
                print("Size of output:")
                theSize = config_myglobals.TansformerOutputAll.size()[0]
                theTargetSize = len([*config_myglobals.TargetOutputsTrain, *config_myglobals.TargetOutputsVal])
                print(theSize)
                print("Size of target:")
                print(theTargetSize)
                if(theSize > theTargetSize) : 
                    print("rezised to")
                    config_myglobals.TansformerOutputAll = config_myglobals.TansformerOutputAll[:theTargetSize, :]
                    print(config_myglobals.TansformerOutputAll.size())
                #vizualize:
                vizualizations_latent.plotTSNEandUMAP(config_myglobals.TansformerOutputAll, 
                                                        [*config_myglobals.TargetOutputsTrain, *config_myglobals.TargetOutputsVal], 
                                                        config_myglobals.WhatEpochWasIt, 
                                                        512*4,  config_myglobals.WhatFold)
                #reset Transformer output to empty for next epoch
                config_myglobals.TansformerOutputAll = torch.from_numpy(np.array([]))
                print("starting new epoch {}".format(config_myglobals.WhatEpochIsIt))
                config_myglobals.WhatEpochWasIt = config_myglobals.WhatEpochIsIt 


        #if last epoch wasn't same as previus we stack to this vector, if it was same this is the first imput to the new epoch
        config_myglobals.TansformerOutputAll = torch.cat((config_myglobals.TansformerOutputAll, TransformerOutput_new), 0)
        
        '''' 

        if(config_myglobals.WhatEpochIsIt == 1) :
            print("epoch 1, size:")
            config_myglobals.TansformerOutputAll = torch.cat((config_myglobals.TansformerOutputAll, TransformerOutput_new), 0)
            print(config_myglobals.TansformerOutputAll.size())

        if(config_myglobals.WhatEpochIsIt == 2) : 
            #plotTSNEandUMAP(TransformerOutputAll, TargetValues, epochNr, dim)
            vizualizations_latent.plotTSNEandUMAP(config_myglobals.TansformerOutputAll, [*config_myglobals.TargetOutputsTrain, *config_myglobals.TargetOutputsVal], config_myglobals.WhatEpochIsIt, 512*4 )
            sys.exit()
        

        if(config_myglobals.WhatEpochIsIt == config_myglobals.TotalNrOfEpochs) : 
            config_myglobals.TansformerOutput = torch.cat((config_myglobals.TansformerOutput, TransformerOutput_new), 0)
            print(config_myglobals.TansformerOutput.size())

        '''

        return TransformerOutput_new
...
class BENDRClassification(Classifier):
    ...
    def features_forward(self, *x):
        ...
        TransformerOutput_new  = context[:, :, -1]
        ...
        if(config_myglobals.WhatEpochIsIt == 2) : 
            ...
            dataFrameLatent = pd.DataFrame(config_myglobals.TansformerOutputAll, columns=loc_cols).astype("float16")
            dataFrameLatent.info(memory_usage='deep')
            dataFrameLatent.to_pickle("./dummyLatentEpoch1.pkl")  
            dataFrameLatent['label'] = [*config_myglobals.TargetOutputsTrain, *config_myglobals.TargetOutputsVal]
            dataFrameLatent.to_pickle("./dummyLatentwithLabelEpock1.pkl")
            dataFrameLatent['label'] = dataFrameLatent['label'].apply(lambda i: str(i))
            dataFrameLatent.to_pickle("./dummyLatentLabelNamesEpoch1.pkl") 
        ...
        return TransformerOutput_new[:, :, -1]
...
class _BENDREncoder(nn.Module):
    ...
    def load(self, filename, strict=True):
        state_dict = torch.load(filename, map_location=torch.device('cpu')) #BREYTING torch.load(filename)
